...
int main() {
	...
	sa.sa_handler = sigHupHandler;    // Хранит указатель на нашу функцию, в которой скрывается обработчик сигнала
	sa.sa_flags |= SA_RESTART;        // Устанавливаем флаг
	sigaction(SIGHUP, &sa, NULL);     // Повторный вызов функции sigaction уже с просьбой нашу модифицированную структуру сохранить в ядре
	...
	// Создаём и настраиваем структуру sockaddr_in для работы с интернет-протоколом (IPv4)
	struct sockaddr_in serverAddress;                      // Заводим переменную типа sigaction
	serverAddress.sin_family = AF_INET;                    // Указываем что используем IPv4 адресацию
	serverAddress.sin_addr.s_addr = htonl(INADDR_ANY);     // Задаём адрес к которому будет привязан сокет, INADDR_ANY позволяет сокету принимать соединения с любого IP-адреса, а htonl преобразует этот адрес из формата хоста в сетевой формат (big-endian)
	serverAddress.sin_port = htons(5555);                  // Устанавливаем номер порта, на котором будет слушать сокет

	bind(serverSocket, (struct sockaddr*)&serverAddress, sizeof(serverAddress));   // Связываем сокет с адресом, указанным в serverAddress, чтобы операционная система знала, на каком IP-адресе и порту будет слушать ваш сервер
	listen(serverSocket, SOMAXCONN);                                               // Переводим сокет в состояние прослушивания 

	printf("Server is running\n");  // Оповещаем о запуске сервера
	...
	bool isConnected = false;           // Ставим isConnected на фолс, так как ещё нет соединения
	...
	while (true)
	{
		...
		if (FD_ISSET(serverSocket, &fds))  // Используем FD_ISSET для проверки, готов ли серверный сокет к обработке нового соединения
                {
		...
		if (FD_ISSET(clientSocket, &fds)) // Используем FD_ISSET для проверки, есть ли данные для чтения из клиентского сокета
		{
			...
			else printf("%zd bits were read from %d\n", readVolume, clientSocket); // Оповещаем о том, сколько байтов информации прочла функция read() из текущего clientSocket
		}
	}
